...
class Ship:  
    ...
    def update_coordinates(self, lset):
...
class Fleet:
    ...
    def __init__(self,visible = True):
        if visible == True:
            ...
            self.carrier = create_ship(5, enemy_board)
            self.battleship = create_ship(4, enemy_board)
            self.destroyer = create_ship(3, enemy_board)
            self.submarine = create_ship(3, enemy_board)
            self.patrol = create_ship(2, enemy_board)
...
def redraw_screen():
    ...
    canvas.blit(text("Press <space> to change orientation of boat.",sf,WHITE),[8 *GRID_SIZE,7 * GRID_SIZE])
...
def create_ship(n, board = Board([25*GRID_SIZE,8*GRID_SIZE])): 
    ...
    lship = Ship(n,board.invgrid(pos),(67 + lset[0],80 + lset[0],80 + lset[0]),1-(lset[1]%10 - lset[0]%10))
    lship.update_coordinates(lship.nset(lset[0]))
    board.update_used(lship.coord) 
    board.remove_unused(lship.coord)
    print(lset)
    return lship
